...
def _training_hparams(cfg: BackboneConfig, *, epochs: int, lr: float, batch_size: int) -> tuple[int, float, int]:
    intervention = cfg.raw.get("intervention", {})
    return (
        int(intervention.get("epochs", epochs)),
        float(intervention.get("lr", lr)),
        int(intervention.get("batch_size", batch_size)),
    )
